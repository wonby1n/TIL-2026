...
Docker 이미지는 여러 레이어가 쌓인 구조다. 이전 빌드와 동일한 단계는 빌드 캐시를 재사용할 수 있다.
...
- 자주 바뀌지 않는 단계를 Dockerfile 위쪽에 두면 캐시를 활용하기 쉽다.
- `.dockerignore`로 `.git`, 로그, 빌드 결과물, `.env` 등을 빌드 컨텍스트에서 제외한다.
- 멀티 스테이지 빌드는 빌드 도구와 실행환경을 분리하여 최종 이미지에 실행에 필요한 파일만 포함한다.

```text
🏗️ 빌드 단계: JDK + Gradle + 소스 → app.jar
                              │
                              ▼
🚀 실행 단계: 작은 Java 실행환경 + app.jar
```

컨테이너 실행 중 만든 변경은 컨테이너의 쓰기 레이어에 저장되며 컨테이너 삭제 시 사라질 수 있다. 유지해야 하는 데이터는 볼륨이나 외부 저장소에 둔다.
